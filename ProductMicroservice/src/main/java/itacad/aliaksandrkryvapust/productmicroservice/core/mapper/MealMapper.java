...
public class MealMapper {
    ...
    public Meal inputMapping(MealDtoInput mealDtoInput, MyUserDetails userDetails) {
        List<Ingredient> ingredient = mealDtoInput.getComposition().stream()
    ...
    public MealDtoOutput outputMapping(Meal meal) {
        List<IngredientDtoOutput> ingredients = meal.getIngredients().stream()
                .map(ingredientMapper::outputMapping)
                .collect(Collectors.toList());
    ...
    public PageDtoOutput<MealDtoOutput> outputPageMapping(Page<Meal> meal) {
        List<MealDtoOutput> outputs = meal.getContent().stream()
